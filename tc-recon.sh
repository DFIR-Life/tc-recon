...
case $2 in 
	...
	;;
	dir)
	dirsearch -u $DOMAIN  -e php -o $DIRECTORY/${DOMAIN}_dirsearch --format=plain
	...
	;;
	who)
    whois $DOMAIN  >>  $DIRECTORY/${DOMAIN}_misc
	nslookup $DOMAIN  >>  $DIRECTORY/${DOMAIN}_misc
	wget $DOMAIN/robots.txt --no-check-certificate   >>  $DIRECTORY/${DOMAIN}_robots
